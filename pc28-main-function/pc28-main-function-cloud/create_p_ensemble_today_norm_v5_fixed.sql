-- 创建或替换 p_ensemble_today_norm_v5 视图 (安全版本)
-- 五桶模型的加权平均计算公式，使用SAFE函数处理数据解析错误
...
WITH
  ...
  -- 地图预测数据 - 使用SAFE函数处理时间戳
  map_pred AS (
    SELECT 
      CAST(period AS STRING) AS period,
      p_win AS p_map,
      vote_ratio AS conf_map
  ...
  -- 尺寸预测数据 - 使用SAFE函数处理时间戳
  size_pred AS (
    SELECT 
      CAST(period AS STRING) AS period,
      p_win AS p_size,
      vote_ratio AS conf_size
  ...
  -- 组合预测数据 - 从实际的combo_based_predictions表获取，使用SAFE函数
  combo_pred AS (
    SELECT 
