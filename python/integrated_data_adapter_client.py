...
# -*- coding: utf-8 -*-
"""
IntegratedDataAdapter (Client Version)
- 集成新的上游API与BigQuery Python客户端适配器
- 提供统一的数据获取接口
- 支持实时数据获取和历史数据回填
- 自动数据同步和验证
"""
...
from bigquery_client_adapter import BQClient
...
class IntegratedDataAdapter:
    ...
    def get_draws_today(self) -> int:
        ...
        if self.fallback_to_bigquery:
            try:
                return self.bq.get_draws_today()
            except Exception as e:
    ...
    def get_kpi_window(self, window_min: int = 60) -> Dict[str, Any]:
        ...
        try:
            return self.bq.get_kpi_window(window_min)
        except Exception as e:
    ...
    def validate_data_consistency(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.fallback_to_bigquery:
                try:
                    bq_draws = self.bq.get_draws_today()
    ...
    def _get_current_draw_from_bigquery(self) -> Optional[Dict[str, Any]]:
        ...
        SELECT 
            ...
            result_digits,
            result_sum
        FROM `{self.bq.project}.{self.bq.ds_draw}.draws_14w_dedup_v`
        WHERE DATE(timestamp, '{self.bq.timezone}') = DATE('2024-12-19')
        ORDER BY timestamp DESC
        LIMIT 1
        """
        
        try:
            results = self.bq._run_query(sql)
            if results:
                result = results[0]
                # 转换为与上游API一致的格式
                return {
    ...
    def _sync_to_bigquery_with_validation(self, data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            # 使用BigQuery客户端进行数据插入
            target_table = f"{self.bq.project}.{self.bq.ds_draw}.draws_realtime"
            ...
            for item in data:
                try:
                    # 转换数据格式以适配BigQuery表结构
                    row_data = {
                        'draw_id': item.get('long_issue'),
                        'timestamp': item.get('kjtime'),
                        'numbers': json.dumps(item.get('number', [])),
                        'sum_value': sum(item.get('number', [])),
                        'is_odd': sum(item.get('number', [])) % 2 == 1,
                        'is_big': sum(item.get('number', [])) >= 14
                    }
                    
                    if self.bq.insert_rows(target_table, [row_data]):
                        success_count += 1
                    else:
                        failed_count += 1
                        errors.append(f"插入失败: {item.get('long_issue')}")
                        
                except Exception as e:
                    failed_count += 1
                    errors.append(f"处理数据异常: {str(e)}")
            
            status = 'success' if failed_count == 0 else ('partial' if success_count > 0 else 'error')
            
            self.logger.info(f"数据同步完成: 状态 {status}, "
